...
const JWT_SECRET = new TextEncoder().encode(
  process.env.AUTH_ADMIN_JWT_SECRET || 'auth-admin-secret-change-in-production'
);
const JWT_EXPIRES_IN = '7d';
const COOKIE_NAME = 'auth_admin_token';
...
export function createAdminAuthMiddleware(prisma: PrismaClient) {
  return async function requireAdminAuth(req: Request, res: Response, next: NextFunction) {
